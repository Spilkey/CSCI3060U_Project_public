    ...
    while (exit == false) {
        ...
        if (curr_user == NULL) {
            ...
            if (acc_type == "AA" && command == "create") {
                // Run create
                error = createUser(curr_user);

            } else if (acc_type == "AA" && command == "delete") {
                error = deleteUser(curr_user);
                ...
                error = sell(curr_user);

            } else if (acc_type != "AA" && command == "addcredit") {
                error  = addCredit_Standard(curr_user);
...
std::string buy(User* curr_user){
  ...
  current_tickets = get_tickets(event_t, seller_username);
